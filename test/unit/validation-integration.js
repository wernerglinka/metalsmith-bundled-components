...
describe( 'Validation Integration', () => {
  // Helper to run plugin with files using real Metalsmith instance
  function runPlugin( files, options = {} ) {
    return new Promise( ( resolve, reject ) => {
      const metalsmith = Metalsmith( fixture( 'validation' ) );
      
      // The plugin function receives files directly - no need to add them to metalsmith
      // This is the proper way: plugin(files, metalsmith, done)
  ...
  describe( 'Valid Sections', () => {
    it( 'should validate correct hero section', async () => {
      ...
      assert( result['assets/main.css'] );
      assert( result['assets/main.css'].contents.toString().includes( 'hero' ) );
    } );
